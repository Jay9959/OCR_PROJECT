...
def is_blank_page(img: Image.Image) -> bool:
    """
    Blank / black / almost empty pages go to BLANK folder.
    Real text/table/document pages stay in temp_fixed.
    """
    gray = np.array(img.convert("L"))
    h, w = gray.shape

    crop = gray[int(h * 0.06):int(h * 0.94), int(w * 0.06):int(w * 0.94)]
    total = crop.size

    white_ratio = np.sum(crop > 245) / total
    dark_ratio = np.sum(crop < 40) / total
    std_dev = float(np.std(crop))
    ...
    for i in range(1, num_labels):
        ...
        # ignore scanner border / long line
        if cw > crop.shape[1] * 0.80 or ch > crop.shape[0] * 0.80:
            continue

        useful_components += 1

    # black blank page
    if dark_ratio > 0.65 and std_dev < 60:
        return True

    # pure white blank page
    if white_ratio > 0.90 and useful_components < 45:
        return True

    # almost empty white page with light dust / border
    if white_ratio > 0.82 and ink_ratio < 0.035 and useful_components < 60:
        return True

    # very empty page
    if ink_ratio < 0.012 and useful_components < 25:
        return True
